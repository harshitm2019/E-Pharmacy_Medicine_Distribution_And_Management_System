        ...
        <Box sx={{ width: SIDEBAR_WIDTH, height: "100%", bgcolor: "#FFFFFF", display: "flex", flexDirection: "column" }}>
            <Box sx={{ height: 80, display: "flex", alignItems: "center", gap: 1.5, px: 3, background: "linear-gradient(135deg,#4CAF50,#2E7D32)" }}>
                ...
                <Box>
                    ...
                    <Typography sx={{ color: "rgba(255,255,255,.8)", fontSize: "12px" }}>Admin Panel</Typography>
                </Box>
            ...
            <List sx={{ px: 2, py: 2, flex: 1 }}>
                {ADMIN_MENUS.map(menu => (
                    <ListItemButton key={menu.title} component={NavLink} 
                    to={menu.path} onClick={onClose} sx={{ mb: 1, borderRadius: "12px", 
                    color: "#455A64", "& .MuiListItemIcon-root": { color: "inherit", minWidth: 42 }, "&.active": { bgcolor: "#E8F5E9", color: "#2E7D32", fontWeight: 700 }, "&:hover": { bgcolor: "#F5F7FA" } }}>
            ...
            <Box sx={{ p: 2 }}>
                <ListItemButton  onClick={logout} sx={{ borderRadius: "12px", color: "#D32F2F", 
                    "&:hover": { bgcolor: "#FFEBEE" } }}>
                    <ListItemIcon sx={{ color: "#D32F2F", minWidth: 42 }}>
                        {LOGOUT_MENU.icon}</ListItemIcon>
