...
@Operation(
        ...
        description = "Returns a specific order by its ID. Accessible by the owner or an ADMIN."
)
@ApiResponses({
        @ApiResponse(responseCode = SwaggerResponses.OK, description = SuccessMessages.ORDER_FETCHED_SUCCESSFULLY),
        @ApiResponse(responseCode = SwaggerResponses.UNAUTHORIZED, description = SwaggerResponses.UNAUTHORIZED_DESC),
        @ApiResponse(responseCode = SwaggerResponses.FORBIDDEN, description = "You do not have permission to view this order"),
...
public @interface GetOrderByIdApi {
}
