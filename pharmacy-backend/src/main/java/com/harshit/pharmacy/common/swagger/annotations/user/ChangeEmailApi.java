...
@Operation(
        ...
        description = "Changes the email address of the currently authenticated user."
)
@ApiResponses({
        @ApiResponse(responseCode = SwaggerResponses.OK, description = SuccessMessages.EMAIL_CHANGED),
        @ApiResponse(responseCode = SwaggerResponses.BAD_REQUEST, description = "Password incorrect or email invalid"),
        @ApiResponse(responseCode = SwaggerResponses.UNAUTHORIZED, description = SwaggerResponses.UNAUTHORIZED_DESC),
        @ApiResponse(responseCode = SwaggerResponses.CONFLICT, description = "Email already in use")
})
